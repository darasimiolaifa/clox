...
VM vm; // optimize to pass VM pointer rather than use global variable
...
static bool callValue(Value calle, int argCount) {
    if (IS_OBJ(calle)) {
        switch (OBJ_TYPE(calle)) {
            case OBJ_BOUND_METHOD: {
                ObjBoundMethod* bound = AS_BOUND_METHOD(calle);
                vm.stackTop[-argCount - 1] = bound->receiver;
                return call(bound->method, argCount);
            }
            case OBJ_CLASS: {
                ObjClass* klass = AS_CLASS(calle);
            ...
            case OBJ_CLOSURE: return call(AS_CLOSURE(calle), argCount);
            case OBJ_NATIVE: {
                NativeFn native = AS_NATIVE(calle);
